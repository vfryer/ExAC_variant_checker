...
#        self.seq_capture_rev = ''.join(seq_list)
    ...
    def wt_allele_construct(self,category):
        ...
        if category == "SN deletion":
            ...
            # Base before '>' in cdna nomeclature, always third character from the end of cDNA
            self.wt_allele_seq = self.cdnachange[-3]
    ...
    def alt_allele_construct(self,category,sequence):
        ...
        if category == "SN deletion":
            ...
            # Base that is the last character of the cDNA nomenclature
            self.alt_allele_seq = self.cdnachange[-1]
...
with open(file+'_exac_capture.csv', 'w') as outfile:
    ...
    for variant in variants:
        ...
        # If the variant is not recorded in ExAC, do not output this row to the file
