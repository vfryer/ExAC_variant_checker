...
with open(file) as infile:
    ...
    for row in csvreader:
        ...
        # handle cDNA nomenclature which spans more than one base or only one base
        if row[4] != row[5]: 
            # handle variants in genes that occur on forward strand (1) and reverse strand (-1)
            if row[0].strand == 1:
...
with open('missense_output.csv', 'wb') as outfile:
