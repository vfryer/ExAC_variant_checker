...
# Define variables for data from input filename.csv
    ...
    def working_coords_capture(self,dist_from_cdna_start, dist_from_cdna_end):
         ...
         if self.strand == 1:
            ...
            self.exac_start_pos = int((self.cdna_start_coord)+int(dist_from_cdna_start))-1
    ...
    def sequence_capture(self):
        ...
        if not r.ok:
            ...
            if self.strand == 1:
                 ...
                 self.seq_capture = r.text[::-1]
                 print(seq_capture)
            else:
                print("Strand not found.\nUnable to execute sequence_capture function.\nProgram will abort.")
    ...
    def wt_allele_construct(self,category):
        ...
        if category == "SN deletion":
            ...
            self.wt_allele_seq = self.exac_start_base + self.sequence_capture(self.seq_capture)
            print(wt_allele_seq)
...
"""
    def exac_capture(self, chr, exac_start_pos, wt_allele_seq, alt_allele_seq):
        '''
        Generate ExAC API list, submit and receive output
        '''
        server = "http://exac.hms.harvard.edu"
        ext = "/rest/variant/variant/" + str(self.chr) + "-" + str(self.exac_start_pos) + "-" + wt_allele_seq + "-" + alt_allele_seq
...
with open(file+'_output.csv', 'w') as outfile:
    ...
    # Add a header row
    csvwriter.writerow(["VARIANT_ID","CDNA_CHANGE", "AACHANGE","GENE", "REFSEQ", "ENSEMBL_TRANSCRIPT", "ENSEMBL_GENE_ID", "VARIANT_STATUS", "ZYGOSITY", "EXON_INTRON", "PROTEIN_EFFECT", "MOI", "VARIANT_CATEGORY", "CDNA_START","DIST_FROM_CDNA_START","CDNA_END","DIST_FROM_CDNA_END","SEQUENCE","CHROMOSOME","STRAND", "CDNA_START_POS", "DNA_START_BASE", "EXAC_START_POS", "EXAC_START_BASE", "CDNA_END_POS", "DNA_END_BASE", "SEQ_CAPTURE","WT_ALLELE_SEQ","ALT_ALLELE_SEQ"])
    # Loop through created variant objects
    for variant in variants:
        row = [variant.variant_id, variant.cdnachange, variant.aachange, variant.gene, variant.refseq, variant.ensembl_transcript, variant.ensembl_gene_id, variant.status, variant.zygosity, variant.exon_intron, variant.protein_effect, variant.mode, variant.category, variant.cdna_start, variant.dist_from_cdna_start, variant.cdna_end, variant.dist_from_cdna_end, variant.sequence, variant.chr, variant.strand, variant.dna_start_pos, variant.dna_start_base, variant.exac_start_pos, variant.exac_start_base, variant.dna_end_pos, variant.dna_end_base, variant.seq_capture, variant.wt_allele_seq, variant.alt_allele_seq]
        # Write row to outfile
        csvwriter.writerow(row)

"""
csvwriter.writerow(["CDNA_CHANGE", "AACHANGE","GENE", "REFSEQ", "ENSEMBL_TRANSCRIPT", "ENSEMBL_GENE_ID", "VARIANT_STATUS", "ZYGOSITY",
"EXON_INTRON", "PROTEIN_EFFECT", "MOI", "VARIANT_CATEGORY", "chromosome", "genomic_start", "genomic_start_calc", "strand", "wt_allele", "alt_allele",
"zygosity", "variant_status", "mode", "refseq_id", "ensembl_gene_id", "exon_intron", "protein_effect", "total_allele_count", "tot_allele_num",
"tot_hom", "tot_allele_freq", "hom_AFR", "hom_NFE", "hom_EA", "hom_FE", "hom_LAT", "hom_Other", "hom_SA", "pop_AFR", "pop_NFE", "pop_EA", "pop_FE",
"pop_LAT", "pop_Other", "pop_SA","pop_total_AFR", "pop_total_NFE", "pop_total_EA", "pop_total_FE", "pop_total_LAT", "pop_total_Other","pop_total_SA",
"freq_AFR", "freq_NFE", "freq_EA", "freq_FE", "freq_LAT", "freq_Other", "freq_SA"])
"""

"""
        row = [variant.cdnachange, variant.aachange, variant.gene, variant.refseq, variant.transcript, variant.ensembl_gene_id, variant.status, 
variant.zygosity, variant.exon_intron, variant.protein_effect, variant.mode, variant.category, variant.chromosome, variant.genomic_start, 
variant.genomic_start_calc, variant.strand, variant.wt_allele_seq, variant.alt_allele_seq, variant.zygosity, variant.variant_status, variant.mode, 
variant.refseq_id, variant. ensembl_gene_id, variant.exon_intron, variant.protein_effect, variant.allele_count, variant.tot_allele_num, 
variant.tot_hom, variant.total_allele_freq, variant.hom_AFR, variant.hom_NFE, variant.hom_EA, variant.hom_FE, variant.hom_LAT, variant.hom_Other, 
variant.hom_SA, variant.pop_AFR, variant.pop_NFE, variant.pop_EA, variant.pop_FE, variant.pop_LAT, variant.pop_Other, variant.pop_SA, 
